...
export default function BusinessLoginPage() {
  ...
  useEffect(() => {
    if (ready && authenticated && walletsReady) {
      if (wallets.length === 0) { router.push('/business/register'); return; }
  ...
  const handleLoginWithCode = async () => {
    ...
    catch (err) { setError(err instanceof Error ? err.message : 'Failed to login'); }
  };

  const handleGoogleLogin = async () => {
    setError('');
    try { initOAuth({ provider: 'google' }); }
    catch (err) { setError(err instanceof Error ? err.message : 'Failed to login with Google'); }
  };
      ...
      <div className="flex w-full max-w-5xl min-h-[680px] rounded-3xl overflow-hidden shadow-lg">
        ...
        <div className="hidden md:flex flex-col justify-between w-[42%] shrink-0 p-14 relative overflow-hidden" style={{ backgroundColor: '#0d1f0d' }}>
          ...
          <div className="relative z-10">
            {/* Headline */}
            <h1 className="relative z-10 text-4xl font-black text-white leading-tight mb-16">
          ...
          <ul className="relative z-10 flex flex-col gap-4">
            {features.map((f) => (
              <li key={f} className="flex items-center gap-3 text-base text-white -mt-1.5">
                <span className="flex items-center justify-center w-5 h-5 rounded-full shrink-0" style={{ backgroundColor: 'rgba(110, 241, 28, 0.2)' }}>
                  <Check className="w-3 h-3" style={{ color: '#5ef500' }} />
                </span>
                {f}
              </li>
            ))}
        ...
        <div className="flex-1 bg-white p-14 flex flex-col justify-center">
          ...
          <p className="text-base text-gray-800 mb-8">Sign in to your PayBridge account.</p>
          ...
          {!isAwaitingCode ? (
            <>
              ...
              >
                {isLoading ? <><Loader2 className="h-4 w-4 animate-spin" />Sending code...</> : 'Sign in'}
              </button>
            ...
            <>
              <div className="mb-4">
                ...
                <input
                  ...
                  disabled={isLoading}
                  onKeyDown={(e) => e.key === 'Enter' && code && handleLoginWithCode()}
                  className="w-full px-4 py-4 rounded-xl border border-black/10 text-base outline-none focus:border-[#9FE870] focus:ring-1 focus:ring-[#9FE870] transition-all bg-white"
                />
              ...
              <button
                onClick={() => { setCode(''); setEmail(''); setError(''); setCodeSent(false); window.location.reload(); }}
                className="w-full py-3 rounded-xl font-medium text-sm border border-black/10 hover:bg-gray-50 transition-all"
                style={{ color: '#163300' }}
              >
          ...
          <p className="text-center text-base text-gray-800 mt-8">
            ...
            <Link href="/business/register" className="font-semibold underline underline-offset-2" style={{ color: '#083400' }}>
              Sign up
            </Link>
          </p>
        </div>
